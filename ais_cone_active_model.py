...
from ais_var_active_cone import *
...
class ActiveConelaminaNeuron():
    """A ball & stick neuron model describing """
    def __init__(self):
        self.Rin = None
        self.label = 'active_cone'
        self.create_sections()
        self.build_topology()
        self.all = h.allsec()
        self.dendrites = [self.dend]
        self.neuron_cable = [self.soma, self.AIS, self.spacer ,self.axon]
        self.define_geometry()
        self.set_biophysics()
        self.add_current_stim()
    def __repr__(self):
        return 'ActiveConelaminaNeuron'
    ...
    def define_geometry(self):
        ...
        for seg, diam in zip(self.spacer, np.linspace(SPACER_DIAM, AIS_DIAM, self.spacer.nseg)):
            seg.diam = diam      
        # AIS
        self.AIS.L = AIS_L
